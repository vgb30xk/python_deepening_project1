...
def setup_gemstone():
    ...
    gemstone_group.add(
        Gemstone(gemstone_images[3], (150, 500), diamod_price, diamond_speed))
...
gemstone_images = [
    ...
    pygame.image.load(os.path.join(current_path, "diamond.png")).convert_alpha()]  # 다이아
...
while running:
    ...
    if not caught_gemstone:  # 잡힌 보석이 없다면 충돌 체크
        for gemstone in gemstone_group:
            ...
            if pygame.sprite.collide_mask(claw, gemstone): # 투명영역 제외하고 실제 이미지부분에 대해 충돌처리
